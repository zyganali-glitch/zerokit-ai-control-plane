...
> Tam yerel çalışma ve browser smoke yolu için Node.js 22 veya üzeri gerekir.
...
3. Final kayıt için görünüyorsa `Ultra` seç; güncel Codex rehberi Ultra'yı maksimum akıl yürütme ve uygun işlerde proaktif alt ajan kullanımı sunan en yüksek zekâ düzeyi olarak tanımlar. Ultra yoksa `Max`, `Extra High` veya `High` gibi ekranda bulunan en yüksek gerçek düzeyi seç.
...
```bash
npm.cmd run codex:record -- ai-buildweek/examples/school-saas.input.md ai-buildweek/runs/school-saas.codex-task.md ai-buildweek/evidence/school-saas.gpt-5.6.codex.config.json --model="GPT-5.6 Sol" --confirm-model-visible --confirm-reviewed
```
